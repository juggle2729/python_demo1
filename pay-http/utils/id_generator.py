...
class LongIdGenerator(object):
    ...
    def generate(self):
        ...
        for component in self.rules:
            lo = component.generate_long()
            id = (id << component.get_length()) + lo
...
_LONG_ID_GENERATOR_DICT = {
    ...
    """
    'pay': LongIdGenerator(instance_id=100 + app.config.get('SERVICE_ID')),
    'jinjian': LongIdGenerator(instance_id=200 + app.config.get('SERVICE_ID')),
    'recharge': LongIdGenerator(instance_id=300 + app.config.get('SERVICE_ID'))
}
